...
rate_crossover = 0.75  # rate that of cross over in population
...
def crossover(parents, target, rate_of_crossover):
    """
    ...
    Thw rate of crossover refers to how many of the parents selected will 
    succesfully create offspring. The parents seleced will create offspring 
    until this rate is reached, at chich point the selected parents will proceed
...
#    x = len(target)/2      # As to split each parent into half
    ...
    r.shuffle(parents)      # Randomises the order so that the choice of parents is random
    y = r.randint(4,len(parents)/2)*2    # chooses random number of parents. Want at least 4 pairs
    #y = int(0.5*len(parents))           # chooses half of the population 
    ...
    while len(new_generation) < len(parents)-len(new_parents)/2:  
        ...
        while len(new_generation) < rate_of_crossover*y:
...
def genetic_algorithm_1(target, size,  length, generations, number_of_winners, mutation_prob, mutation_rate, rate_of_cross):
    """
    ...
    while i < generations:
        ...
        new_gen = crossover(winners, target, rate_of_cross)
...
def genetic_algorithm_2(target, size,  length, number_of_winners, mutation_prob, mutation_rate, rate_of_cross):
    """
...
#genetic_algorithm_1(target_string, pop_size, length_target, generations,  n, prob, rate, rate_crossover)

genetic_algorithm_2(target_string, pop_size, length_target, n, prob, rate, rate_crossover)
